 ...
 *
 * This module supports two prover backends:
 *
 * 1. HASH_BASED (default fallback):
 *    A hash-based proof of knowledge scheme using Fiat-Shamir.
 *    Not a real zero-knowledge proof, but demonstrates the architecture
 *    and provides functional proofs for testing/development.
 *
 * 2. STARK_WASM:
 *    A real STARK proof generated by the Rust prover compiled to WASM.
 *    When the WASM module is loaded (via wasm-loader.ts), this backend
 *    produces cryptographically sound zero-knowledge proofs.
 *
 * The backend selection logic:
 *   - If STARK_WASM is requested and WASM is available, use it.
 *   - Otherwise, fall back to HASH_BASED automatically.
 *   - The caller can force a specific backend via the options parameter.
 *
 * Hash-based proof serialization format (version 0x01, fits in 2048 bytes):
 *   [1 byte]              version (0x01)
...
import { isWasmAvailable, getWasmProver } from './wasm-loader.js';
import { generateStarkProof, serializeForClarity } from './stark-prover.js';
 ...
 *
 * - HASH_BASED: Fiat-Shamir hash-based proof (always available)
 * - STARK_WASM: Real STARK proof via Rust WASM module (requires loading)
 */
export enum ProverBackend {
  /** Hash-based Fiat-Shamir proof (fallback, always available). */
  HASH_BASED = 'hash-based',
  /** Real STARK proof via the Rust WASM prover module. */
  STARK_WASM = 'stark-wasm',
}
...
export interface ProverOptions {
  /** Which backend to use. Defaults to STARK_WASM with HASH_BASED fallback. */
 ...
 *
 * When `options.backend` is `STARK_WASM` (or not specified) and the WASM
 * prover module has been loaded, this function delegates to the Rust STARK
 * prover. Otherwise it falls back to the hash-based Fiat-Shamir scheme.
 *
 * Steps (hash-based backend):
 ...
 *
 * Steps (STARK WASM backend):
 *   1. Marshal witness data to WASM-compatible format.
 *   2. Call the Rust STARK prover via WASM.
 *   3. Serialize the raw STARK proof into the Clarity buffer format.
 *
 * @param witness - A valid witness (produced by generateWitness)
 * @param options - Optional backend configuration
...
export async function generateWithdrawalProof(
  ...
  const requestedBackend = options?.backend ?? ProverBackend.STARK_WASM;

  // -----------------------------------------------------------------------
  // Try STARK WASM backend if requested and available
  // -----------------------------------------------------------------------
  if (requestedBackend === ProverBackend.STARK_WASM && isWasmAvailable()) {
    const wasmProver = getWasmProver();
    if (wasmProver !== null) {
      const starkResult = await generateStarkProof(witness, wasmProver);
      const serialized = serializeForClarity(starkResult.proof);
      return {
        proof: serialized,
        publicInputs: starkResult.publicInputs,
      };
    }
  }

  // -----------------------------------------------------------------------
  // Fall back to hash-based prover
  // -----------------------------------------------------------------------
  return generateHashBasedProof(witness);
}

/**
 * Generate a hash-based Fiat-Shamir proof (the original fallback backend).
 *
 * @param witness - A valid witness (produced by generateWitness)
 * @returns Serialized proof and public inputs
 */
function generateHashBasedProof(witness: Witness): StarkProof {

...
let processorTimer: ReturnType<typeof setTimeout> | undefined;
...
async function processNext(): Promise<void> {
  ...
  try {
    ...
    if (item.type === 'deposit') {
      txId = await submitDepositTx(
        item.payload as DepositPayload,
        config!,
        logger,
      );
    } else {
      txId = await submitWithdrawalTx(
        item.payload as WithdrawalPayload,
        config!,
        logger,
      );
    }
